...
from hiagi.self_supervised.losses.contrastive_loss import byol_contrastive_loss
from hiagi.self_supervised.utils.network_utils import add_weight_decay


def self_supervised_training(network: tf.keras.models.Model,
projector: tf.keras.models.Model,
predictor: tf.keras.models.Model,
                             train_dist_dataset,
    ...
    with strategy.scope():
        ...
        def compute_loss(hidden, temperature):
            per_example_loss = byol_contrastive_loss(hidden)
        ...
        def train_step(inputs):
            with tf.GradientTape() as tape:
                ...
                embedding_1 = network(x)
                proj_out_1 = projector(embedding_1)
                pred_out_1 = predictor(proj_out_1)

                embedding_2 = network(x)
                proj_out_2 = projector(embedding_2)
                pred_out_2 = predictor(proj_out_2)

                hidden = tf.concat([pred_out_1, tf.stop_gradient(proj_out_2)], axis=0)
